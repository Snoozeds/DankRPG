...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(pickaxe === 1) {
            ...
            embed.setDescription(`<@${user.id}> mined some stone and got ${stone} stone!${await get(`${interaction.user.id}_xp_alerts`) == '1' ? `\n+${xp}XP` : ''} ${await checkXP(interaction.user.id, xp) == true ? ` :up: **Level up!** Check /levels.` : ''}`)
            ...
            await interaction.reply({"content": "You don't have a pickaxe to mine!\nYou can /craft one with 25 wood and 50 stone.", "ephemeral": true});
        }
    }
}