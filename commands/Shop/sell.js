...
const { get, set, decr, coinEmoji } = require("../../globals.js");
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (get(user.id, `${selectedItem.name}.amount`) < amount) {
        return interaction.reply({
    ...
    if (await get(user.id, `${selectedItem.name}.amount`) >= amount) {
        await decr(user.id, `${selectedItem.name}`, amount);
        await set(user.id, "coins", await get(user.id, "coins") + selectedItem.value * amount);
        return interaction.reply({
