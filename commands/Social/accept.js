  ...
  async execute(interaction) {
    ...
    if (marriageRequest == null) {
      return interaction.reply({
        content: "You don't have any active marriage requests!",
        ephemeral: true,
      });
    } else {
      ...
      if ((await get(`${marriageRequest}_marriageStatus`)) === "married") {
        return interaction.reply({
          content: "This user has married to another user since their request.",
          ephemeral: true,
        });
      }
      ...
      if ((await get(`${user.id}_sender`)) === "true") {
        return interaction.reply({
