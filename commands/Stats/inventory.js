...
module.exports = {
    ...
    async execute(interaction) {
        ...
        await get(`${user.id}_lifesaver`) === null ? set(`${user.id}_lifesaver`, 0) : null;
        await get(`${user.id}_diamond`) === null ? set(`${user.id}_diamond`, 0) : null;
        await get(`${user.id}_wood`) === null ? set(`${user.id}_wood`, 0) : null;
        await get(`${user.id}_stone`) === null ? set(`${user.id}_stone`, 0) : null;
        const embed = new EmbedBuilder()
        .setTitle(`${interaction.user.username}'s Inventory`)
        .setDescription(`Lifesavers: ${await get(`${user.id}_lifesaver`)}
